...
const AboutUs = () => {
 ...
 return (
    <section
      id="about-us-section"
      className="py-16 bg-gradient-to-br from-amber-50 to-stone-100 dark:from-gray-900 dark:to-gray-800"
    >
      <div className="container mx-auto px-4 max-w-7xl">
        ...
        <motion.div
          ...
          viewport={{ once: true }}
        >
          <h1 className="text-4xl md:text-5xl font-bold text-amber-900 dark:text-amber-100 mb-4">Conoce Zensorial</h1>
          <p className="text-amber-700 dark:text-amber-300 text-lg max-w-3xl mx-auto">
        ...
        >
          {stats.map((stat, index) => (
            <div key={index} className="text-center bg-white dark:bg-gray-800 rounded-xl p-6 shadow-lg">
              <stat.icon className="w-8 h-8 text-amber-600 dark:text-amber-400 mx-auto mb-3" />
              <div className="text-3xl font-bold text-amber-900 dark:text-amber-100 mb-1">{stat.value}</div>
              <div className="text-amber-700 dark:text-amber-300 text-sm">{stat.label}</div>
            </div>
          ))}
        </motion.div>
        ...
        <div className="grid lg:grid-cols-2 gap-16">
                ...
                <button
                  ...
                  className={`flex items-center gap-2 px-4 py-2 rounded-lg font-medium transition-all ${
                    activeTab === tab.id
                      ...
                      : "bg-white dark:bg-gray-800 text-amber-600 dark:text-amber-400 hover:bg-amber-50 dark:hover:bg-gray-700"
                  }`}
                >
          ...
          <div className="space-y-8">
            ...
            <motion.div
              ...
              viewport={{ once: true }}
            >
              <h3 className="text-2xl font-bold text-amber-900 dark:text-amber-100 mb-6">Contáctanos</h3>
              <div className="space-y-4">
                {contactInfo.map((contact, index) => (
                  <motion.a
                    ...
                    className="flex items-center gap-4 p-3 rounded-lg hover:bg-amber-50 dark:hover:bg-gray-700 transition-colors group"
                    whileHover={{ x: 5 }}
                  >
                    <div
                      className={`w-10 h-10 rounded-full bg-gray-100 dark:bg-gray-700 flex items-center justify-center ${contact.color}`}
                    >
                      <contact.icon className="w-5 h-5" />
                    </div>
                    <div>
                      <div className="font-medium text-gray-900 dark:text-gray-100">{contact.label}</div>
                      <div className="text-gray-600 dark:text-gray-400 text-sm">{contact.value}</div>
                    </div>
                  </motion.a>
                ))}
              </div>
            ...
            >
              <div className="flex items-center gap-3 mb-6">
                ...
                <h3 className="text-2xl font-bold text-amber-900 dark:text-amber-100">Horarios de Atención</h3>
              </div>
              <div className="space-y-3">
                {schedule.map((item, index) => (
                  ...
                  >
                    ...
                    <span className="text-amber-600 dark:text-amber-400 font-semibold">{item.hours}</span>
                  </div>
                ))}
              </div>
            </motion.div>
            ...
            >
              <div className="flex items-center gap-3 mb-6">
                ...
                <h3 className="text-2xl font-bold text-amber-900 dark:text-amber-100">Nuestra Ubicación</h3>
              </div>
